<template>
  <ListPageLayout
    ...
    :subtitle="`${filtered.length} demande(s) de stage`"
    :columns="columns"
    :items="pageItems"
    :total="totalCount"
    ...
    <template #above-table>
      <div class="grid grid-cols-3 gap-2.5 mb-3.5 max-md:grid-cols-1">
        <div :class="kpiItem">
          ...
          <div><div :class="kpiVal">{{ applicationStore.internships.length }}</div><div :class="kpiLbl">Total</div></div>
        </div>
        <div :class="kpiItem">
...
/**
 * Liste des demandes de stage (Application, source "Internship"), module
...
/* ── KPIs ───────────────────────────────────────────────────── */
const newCount = computed(() => applicationStore.internships.filter(a => a.status === 'New').length)
const inReviewCount = computed(() => applicationStore.internships.filter(a => a.status === 'InReview').length)
...
const filtered = computed(() => {
  let rows = applicationStore.internships.filter(a => {
